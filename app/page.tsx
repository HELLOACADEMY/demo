...
const PROGRAMS = [
  ...
  { IconComp: Monitor, title: 'Komputer & Coding', desc: 'Belajar dasar-dasar pemrograman dan teknologi untuk masa depan.', color: '#3B82F6', bg: '#DBEAFE', age: 'SD – SMA' },
...
const BRANCHES = [
  { name: 'Cabang Khatulistiwa', address: 'Jl. Khatulistiwa No. 88, Pontianak Utara', phone: '0857-5085-8821', hours: 'Senin – Sabtu: 07.00 – 20.00', color: '#2563EB' },
  { name: 'Cabang Karya Baru', address: 'Jl. Karya Baru No. 45, Pontianak Barat', phone: '0857-5085-8821', hours: 'Senin – Sabtu: 07.00 – 20.00', color: '#10B981' },
  { name: 'Cabang Sungai Raya', address: 'Jl. Sungai Raya Dalam No. 12, Kubu Raya', phone: '0857-5085-8821', hours: 'Senin – Sabtu: 07.00 – 20.00', color: '#F59E0B' },
];

const WHY_ITEMS = [
  { IconComp: Layers, title: 'Kurikulum Terkini', desc: 'Mengikuti kurikulum nasional terbaru (Merdeka Belajar)', color: '#2563EB', bg: 'rgba(37,99,235,0.2)' },
...
const ABOUT_POINTS = [
  { IconComp: CheckCircle2, text: 'Metode belajar berbasis penelitian modern', color: '#2563EB' },
...
const GALLERY_PHOTOS = [
  {
    ...
    title: 'Siswa Membaca & Berhitung Ceria',
    category: 'Pembelajaran Interaktif',
    desc: 'Siswa kelas dasar belajar membaca dan menulis dengan penuh konsentrasi dan bimbingan guru yang ramah.'
  },
  {
    id: 2,
    src: '/gallery/photo2.jpg',
    title: 'Antusiasme Menunjukkan Modul Belajar',
    category: 'Kegiatan Bersama',
    desc: 'Keceriaan para siswa saat menunjukkan buku latihan modul Bsmart Education secara serentak di kelas.'
  },
  {
    id: 3,
    src: '/gallery/photo3.jpg',
    title: 'Keceriaan Kelas TK Pelangi Akasa',
    category: 'Kegiatan Bersama',
    desc: 'Momen penuh tawa dan kehangatan antara siswa dan guru saat sesi pembelajaran interaktif.'
  },
  {
    id: 4,
    src: '/gallery/photo4.jpg',
    title: 'Proyek Gambar & Mewarnai Kelompok',
    category: 'Proyek Kreatif',
    desc: 'Siswa bekerja sama melatih kreativitas seni melukis dan mewarnai lembaran karya gambar besar.'
  },
  {
    id: 5,
    src: '/gallery/photo5.jpg',
    title: 'Prakarya 3D Tata Surya & Antariksa',
    category: 'Proyek Kreatif',
    desc: 'Hasil karya siswa membuat maket 3D planet dan roket antariksa dalam proyek Sains Terpadu.'
  }
];
...
export default function BsmartLandingPage() {
  ...
  const [activeGalleryCategory, setActiveGalleryCategory] = useState('Semua');
  ...
  return (
    <div style={{ background: '#F8FAFC', color: '#0F172A', minHeight: '100vh', fontFamily: "'Nunito', sans-serif", overflowX: 'hidden' }}>
      ...
      <header style={{
        ...
        borderBottom: scrolled ? '1.5px solid #E2E8F0' : 'none',
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', height: '76px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '24px' }}>
          ...
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none', flexShrink: 0 }}>
            ...
            <div>
              <div style={{ fontSize: '1.1rem', fontWeight: 900, color: scrolled ? '#0F172A' : '#fff', lineHeight: 1, letterSpacing: '-0.02em', transition: 'color 0.3s' }}>
                Bsmart <span style={{ color: '#2563EB' }}>Education</span>
              </div>
              <div style={{ fontSize: '0.65rem', fontWeight: 600, color: '#60A5FA', letterSpacing: '0.12em', marginTop: '2px' }}>PONTIANAK</div>
            </div>
          </Link>
          ...
          <nav className="desktop-only" style={{ gap: '6px', alignItems: 'center' }}>
            {[['#hero', 'Beranda'], ['#programs', 'Program'], ['#about', 'Tentang'], ['#branches', 'Cabang'], ['#contact', 'Kontak']].map(([href, label]) => (
              <a key={href} href={href} style={{
                ...
                color: scrolled ? '#1E3A8A' : '#fff', textDecoration: 'none',
                transition: 'all 0.2s ease',
              }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(37,99,235,0.12)'; e.currentTarget.style.color = '#1D4ED8'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = scrolled ? '#1E3A8A' : '#fff'; }}
              >{label}</a>
            ))}
          </nav>
          ...
          <button className="mobile-only" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: scrolled ? '#1D4ED8' : '#fff', padding: '8px' }}>
      ...
      {mobileMenuOpen && (
        <div style={{
          position: 'fixed', inset: 0, zIndex: 99, background: 'rgba(15,23,42,0.97)',
      ...
      <section id="hero" style={{
        minHeight: '100vh', background: 'linear-gradient(135deg, #0F172A 0%, #1E3A8A 40%, #1D4ED8 70%, #2563EB 100%)',
        position: 'relative', display: 'flex', alignItems: 'center', overflow: 'hidden', paddingTop: '80px',
      }}>
        {/* Decorative circles */}
        <div style={{ position: 'absolute', top: '8%', right: '8%', width: '380px', height: '380px', borderRadius: '50%', background: 'rgba(37,99,235,0.25)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '60px 32px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px', alignItems: 'center', width: '100%', position: 'relative', zIndex: 2 }}>
          ...
          <div className="animate-slide-left" style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'relative' }}>
            ...
            <p style={{ fontSize: '1.1rem', color: 'rgba(255,255,255,0.9)', lineHeight: 1.75, fontWeight: 400, maxWidth: '560px' }}>
              Karena setiap anak memiliki potensi istimewa. Selama 12+ tahun, Bsmart Education mendedikasikan pengajaran berkualitas tertinggi, bimbingan hangat ramah anak, dan pantauan progres transparan untuk mengantarkan putra-putri Anda lulus di sekolah & PTN impian.
            </p>

            <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', marginTop: '6px' }}>
              <Link href="/ppdb" className="btn btn-yellow" style={{ fontSize: '1rem', padding: '15px 30px', boxShadow: '0 8px 32px rgba(245,158,11,0.5)', borderRadius: '14px' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>
          <svg viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block' }}>
            <path d="M0,40 C360,80 720,0 1080,40 C1260,60 1380,30 1440,40 L1440,80 L0,80 Z" fill="#F8FAFC" />
          </svg>
        </div>
      </section>
      ...
      <section id="about" style={{ background: '#fff', padding: '80px 32px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-80px', right: '-80px', width: '360px', height: '360px', borderRadius: '50%', background: 'rgba(37,99,235,0.06)', pointerEvents: 'none' }} />

        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          {/* Left image side */}
          <div style={{ position: 'relative' }}>
            <div style={{ borderRadius: '32px', overflow: 'hidden', background: 'linear-gradient(135deg, #DBEAFE, #BFDBFE)', padding: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '380px' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div style={{ position: 'absolute', bottom: '-20px', right: '-20px', background: '#fff', borderRadius: '20px', padding: '18px 24px', boxShadow: '0 12px 40px rgba(37,99,235,0.18)', border: '1.5px solid #DBEAFE', display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <div>
                ...
                <div style={{ fontWeight: 400, fontSize: '0.8rem', color: '#2563EB' }}>Sejak 2012</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <h2 style={{ fontSize: '2.4rem', fontWeight: 900, color: '#0F172A', lineHeight: 1.2 }}>
              Setiap Anak{' '}
              <span style={{ color: '#2563EB' }}>Istimewa</span>{' '}
              & Berhak Sukses
            </h2>
              ...
              {ABOUT_POINTS.map((item, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    ...
                    <span style={{ fontWeight: 500, color: '#1E3A8A', fontSize: '0.95rem' }}>{item.text}</span>
                  </div>
                );
              })}
            </div>
      ...
      {/* ===== PROGRAMS SECTION ===== */}
      <section id="programs" style={{ background: '#F8FAFC', padding: '80px 32px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '52px' }}>
            ...
            <h2 style={{ fontSize: '2.4rem', fontWeight: 900, color: '#0F172A' }}>
              Belajar dengan <span style={{ color: '#2563EB' }}>Cara yang Efektif</span>
            </h2>
            ...
            {PROGRAMS.map((p, i) => {
              ...
              return (
                <div key={i} className="program-card hover-lift">
                  ...
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0F172A', marginBottom: '10px' }}>{p.title}</h3>
      ...
      {/* ===== 📷 MODERN ANIMATED PHOTO GALLERY SECTION ===== */}
      <section id="gallery" style={{ background: '#ffffff', padding: '88px 32px', position: 'relative' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <h2 style={{ fontSize: '2.5rem', fontWeight: 700, color: '#0F172A', lineHeight: 1.2 }}>
              Suasana Belajar <span style={{ color: '#2563EB' }}>Kreatif & Ceria</span> di Bsmart Education
            </h2>
            <p style={{ fontSize: '1rem', color: '#64748B', maxWidth: '620px', margin: '14px auto 0', fontWeight: 400, lineHeight: 1.7 }}>
              Dokumentasi real kegiatan anak-anak dalam pembelajaran interaktif, prakarya sains kreatif, karya seni melukis, serta kebersamaan hangat di cabang Pontianak.
            </p>

            {/* Filter Category Tabs */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '28px', flexWrap: 'wrap' }}>
              {['Semua', 'Pembelajaran Interaktif', 'Proyek Kreatif', 'Kegiatan Bersama'].map((cat) => {
          ...
          {/* Infinite Horizontal Side-Flowing Photo Carousel (Mengalir ke Samping) */}
          <div className="gallery-marquee-container" style={{ width: '100%', overflow: 'hidden', padding: '12px 0 24px', position: 'relative' }}>
            <div className="gallery-marquee-track">
              {[...GALLERY_PHOTOS, ...GALLERY_PHOTOS, ...GALLERY_PHOTOS].map((photo, index) => (
                <div
                  key={`marquee-${photo.id}-${index}`}
                ...
                >
                  ...
                  <div style={{ position: 'relative', overflow: 'hidden', height: '260px', background: '#0F172A' }}>
                    ...
                    />
                    <div style={{ position: 'absolute', top: '14px', left: '14px', background: 'rgba(15, 23, 42, 0.75)', backdropFilter: 'blur(8px)', color: '#ffffff', padding: '6px 14px', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 600 }}>
                      🏷️ {photo.category}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
                    ...
                    <p style={{ fontSize: '0.825rem', color: '#64748B', margin: 0, lineHeight: 1.5, fontWeight: 400 }}>
                      {photo.desc}
                    </p>
                    <div style={{ marginTop: '8px', fontSize: '0.775rem', fontWeight: 600, color: '#2563EB', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {/* 🖼️ LIGHTBOX FULLSCREEN PHOTO MODAL */}
      {selectedPhotoModal && (
        <div
        ...
        >
          ...
          >
            ...
            <div style={{ width: '100%', height: '480px', background: '#0F172A', position: 'relative' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
            ...
            <div style={{ padding: '24px 32px', background: '#ffffff' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#2563EB', background: '#DBEAFE', padding: '4px 12px', borderRadius: '999px' }}>
                {selectedPhotoModal.category}
              </span>
              <h3 style={{ fontSize: '1.4rem', fontWeight: 600, color: '#0F172A', margin: '10px 0 6px' }}>
                {selectedPhotoModal.title}
              </h3>
              <p style={{ fontSize: '0.925rem', color: '#64748B', margin: 0, lineHeight: 1.6 }}>
                {selectedPhotoModal.desc}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ===== WHY CHOOSE US ===== */}
      <section style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E3A8A 40%, #1D4ED8 100%)', padding: '80px 32px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-60px', left: '-60px', width: '300px', height: '300px', borderRadius: '50%', background: 'rgba(96,165,250,0.15)', pointerEvents: 'none' }} />
      ...
      <section id="branches" style={{ background: '#fff', padding: '80px 32px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '52px' }}>
            ...
            <h2 style={{ fontSize: '2.4rem', fontWeight: 900, color: '#0F172A' }}>
              Cabang Kami di <span style={{ color: '#2563EB' }}>Pontianak</span>
            </h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {BRANCHES.map((b, i) => (
              <div key={i} className="glass-card hover-lift" style={{ padding: '32px', position: 'relative', overflow: 'hidden' }}>
                ...
                <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0F172A', marginBottom: '16px' }}>{b.name}</h3>
                ...
                <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1.5px solid #EFF6FF' }}>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="testimonial-card" style={{ opacity: i === activeTestimonial ? 1 : 0.75, transform: i === activeTestimonial ? 'scale(1.03)' : 'scale(1)', transition: 'all 0.4s ease' }}>
                <Quote size={28} color="#DBEAFE" style={{ marginBottom: '16px' }} />
                <p style={{ fontSize: '0.95rem', color: '#1E3A8A', lineHeight: 1.75, fontWeight: 400, marginBottom: '24px', fontStyle: 'italic' }}>"{t.text}"</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px', paddingTop: '20px', borderTop: '1.5px solid #EFF6FF' }}>
                  {/* Initials avatar — no emoji */}
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: 'linear-gradient(135deg, #2563EB, #1D4ED8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                    {t.initials}
                  </div>
                  <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '36px' }}>
            {TESTIMONIALS.map((_, i) => (
              <button key={i} onClick={() => setActiveTestimonial(i)} style={{ width: i === activeTestimonial ? '28px' : '10px', height: '10px', borderRadius: '999px', background: i === activeTestimonial ? '#2563EB' : '#BFDBFE', border: 'none', cursor: 'pointer', transition: 'all 0.3s ease' }} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="contact" style={{ background: '#fff', padding: '80px 32px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ background: 'linear-gradient(135deg, #1E3A8A 0%, #1D4ED8 50%, #2563EB 100%)', borderRadius: '40px', padding: '60px 80px', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '40px' }}>
            ...
            <div style={{ position: 'absolute', bottom: '-60px', left: '5%', width: '240px', height: '240px', borderRadius: '50%', background: 'rgba(96,165,250,0.15)', pointerEvents: 'none' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', position: 'relative', zIndex: 2, flexShrink: 0 }}>
              ...
              <a href="https://wa.me/6285750858821?text=Halo%20Bsmart%20Education,%20saya%20ingin%20konsultasi%20pendaftaran" target="_blank" rel="noreferrer" className="btn btn-outline" style={{ padding: '14px 32px', fontSize: '1rem', justifyContent: 'center' }}>
                <Phone size={18} /> Hubungi Kami via WA
              </a>
            </div>
          </div>
        </div>
      ...
      {/* ===== FOOTER ===== */}
      <footer style={{ background: '#0F172A', color: '#fff', padding: '60px 32px 32px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1.5fr', gap: '48px', marginBottom: '48px' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.7rem', fontWeight: 500, color: '#60A5FA', letterSpacing: '0.1em' }}>PONTIANAK</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                {[
                  { icon: <Share2 size={18} />, color: '#EC4899', href: 'https://www.instagram.com/bsmart_education/', label: 'Instagram' },
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[['#about', 'Tentang Kami'], ['#programs', 'Program Belajar'], ['#branches', 'Lokasi Cabang'], ['/ppdb', 'Daftar Siswa Baru'], ['/login', 'Login Portal']].map(([href, label]) => (
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(96,165,250,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
